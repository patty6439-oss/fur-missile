...
function WeatherPanel({ location, compact = false }) {
  ...
  if (compact) {
    return (
      <section>
        {!location && <p>Weather unavailable.</p>}

        {error && <p>{error}</p>}

        {weather && (
          <p>
            Current Weather: {weather.temperature_2m}°F |{" "}
  ...
  return (
    <section>
      ...
      {!location && (
        <p>Add a mission location to see weather.</p>
      )}
