...
class Dogs(APIView): 
    ...
    def get(self, request): 
        dogs = WorkingDog.objects.filter(owner=request.user).order_by("name") 
...
class DogDetail(APIView): 
    ...
    def get(self, request, dog_id): 
        dog = self.get_dog(request, dog_id) 
        return Response(WorkingDogSerializer(dog).data) 
 
    def put(self, request, dog_id): 
        dog = self.get_dog(request, dog_id) 
        serializer = WorkingDogSerializer(dog, data=request.data, partial=True) 
